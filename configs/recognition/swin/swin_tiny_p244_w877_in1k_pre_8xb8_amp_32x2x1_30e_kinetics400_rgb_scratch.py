...
train_dataloader = dict(batch_size=1, num_workers=0, dataset=dict(type='VideoDataset', ann_file='data/mini_kinetics/train_list.txt', data_prefix=dict(video=''), pipeline=train_pipeline))
val_dataloader = dict(batch_size=1, num_workers=0, dataset=dict(type='VideoDataset', ann_file='data/mini_kinetics/val_list.txt', data_prefix=dict(video=''), pipeline=val_pipeline, test_mode=True))
